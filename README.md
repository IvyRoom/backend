...
The handle remains valid for its absolute four-hour lifetime. The backend has
no authoritative session store, session cookie, current-session endpoint, or
revocation API. Browser logout is therefore local to the frontend; it does not
send a backend logout request or invalidate an already issued handle.
